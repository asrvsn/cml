...
class CML_boiling:
	def __init__(self, shape: Tuple, 
			...
			eps: float, sigma: float, alpha: float, eta2: float, T_c: float,
			dt: float=1.0
		):
		'''
		shape: (m, n) grid
	...
	def density(self, T):
		return np.tanh(self.alpha*(T+self.temp-self.T_c))
	...
	def step(self):
		...
		for y in range (1, m-1):
			for x in range(n):
				mult = 0.
				if T[y, x]+self.temp > self.T_c and self.T[y, x]+self.temp < self.T_c:
					mult = -1.
				elif T[y, x]+self.temp < self.T_c and self.T[y, x]+self.temp > self.T_c:
					mult = 1.
	...
	def reset(self):
		...
		self.T = np.random.normal(size=self.shape, scale=1e-3)
		self.T[0,:] = -self.temp
		self.T[-1,:] = self.temp
...
if __name__ == '__main__':
	...
	model = CML_boiling((m, n), temp, nu, eta1, kappa, c, eps, sigma, alpha, eta2, T_c)
